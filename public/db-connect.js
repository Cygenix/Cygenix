...
exports.handler = async function (event) {
  ...
  try {
    ...
    switch (action) {
      ...
      // Paginated SELECT — returns one page of rows at a time using OFFSET/FETCH
      case 'fetch-page': {
        const { sql: pageSql, offset = 0, pageSize = 2000 } = body;
        if (!pageSql) return err('sql is required for fetch-page', null, 400);

        // Wrap the user query in an OFFSET/FETCH if it doesn't already have one
        // Strip trailing semicolon, wrap with pagination
        const baseQuery = pageSql.replace(/;\s*$/, '').trim();

        // Build paged query — requires an ORDER BY, so add one if missing
        const hasOrderBy = /ORDER\s+BY/i.test(baseQuery);
        const orderedQuery = hasOrderBy
          ? baseQuery
          : `${baseQuery} ORDER BY (SELECT NULL)`;
        const pagedQuery = `${orderedQuery} OFFSET ${parseInt(offset)} ROWS FETCH NEXT ${Math.min(parseInt(pageSize),5000)} ROWS ONLY`;

        const r = await pool.request().query(pagedQuery);
        result = {
          success: true,
          offset: parseInt(offset),
          pageSize: Math.min(parseInt(pageSize), 5000),
          rows: r.recordset || [],
          rowCount: (r.recordset || []).length,
          hasMore: (r.recordset || []).length >= Math.min(parseInt(pageSize), 5000)
        };
        break;
      }

      // Count rows for a given query or table — used before paginating
      case 'count': {
        const { sql: countSql } = body;
        if (!countSql) return err('sql is required for count', null, 400);
        const baseQ = countSql.replace(/;\s*$/, '').trim();
        const r = await pool.request().query(`SELECT COUNT(*) AS total_count FROM (${baseQ}) AS _count_wrap`);
        result = { success: true, count: r.recordset?.[0]?.total_count || 0 };
        break;
      }
      ...
      default:
        return err(`Unknown action: ${action}`, 'Valid actions: test | schema | execute | fetch-page | count | batch | rowcounts', 400);
    }
