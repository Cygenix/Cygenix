               ...
               'data_stream_store.html', 'data_stream_monitor.html'];
{
  ...
  check('with all four children pointing at their pages',
    ['data_stream.html', 'data_stream_store.html', 'data_stream_events.html', 'data_stream_monitor.html']
  ...
  check('but the Streams screen links to it',
    /data_stream_designer\.html/.test(read('public', 'data_stream.html')));
...

{
  ...
  check('the Schema Explorer can send a table to a new stream',
    /Add to data stream/.test(se) && /data_stream_designer\.html/.test(se));
