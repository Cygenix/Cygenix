...
check('AND SO DOES SIMPLY OPENING THE CONNECTIONS VIEW, because that tab is the default',
  ...
  // connCloudStatus() (Sep-2026) and the organisation register's first
  // draw (Phase A) render between the env note and the mount; the card
  // still mounts on every open of the view, which is the claim.
  && /connRenderEnv\(\);\s*(connCloudStatus\(\);\s*)?(orgConnRender\(\);\s*)?(orgConnEnsureRoles\(\);\s*)?(\/\/[^\n]*\n\s*)*(try \{ if \(window\.CygenixOrgConnections\)[^\n]*\n\s*)?\/\/ The Collation card mounts here as well as in switchConnTab\./.test(APP));
